::
::  VENDORED from upstream `vesl/protocol/lib/vesl-stark-verifier.hoon`.
::
::  Why this file is NOT a symlink into `$VESL_HOME` like its four
::  neighbours: it carries a local patch — the `?=(%& -.result)`
::  narrowing at `++verify` — that newer `hoonc` requires but that
::  hasn't been merged upstream yet. Without it every server that
::  symlinks to a fresh `vesl/main` hits a nest-fail ~90 seconds into
::  the kernel compile. See `scripts/setup-hoon-tree.sh` for the
::  symlink-exclusion logic.
::
::  Upstream patch sits on the local `phase1-verifier-debug-and-type-fix`
::  branch at commits `dc9382c` + `498f3ca`. When those land in
::  upstream `vesl/main`, replace this file with a symlink and drop
::  the exclusion in `setup-hoon-tree.sh`.
::
::  Refresh-from-upstream procedure (when you re-sync Vesl):
::    1. cp $VESL_HOME/protocol/lib/vesl-stark-verifier.hoon hoon/lib/
::    2. re-apply the `?=(%& -.result)` narrowing at `++verify`
::    3. re-add this header banner
::    4. `make install` and confirm tests pass
::
