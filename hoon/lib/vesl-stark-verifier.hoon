...
::
::  +verify-raw: same as +verify but does NOT swallow verify-inner
::  crashes with mule. Used for Phase 1-redo debugging so assert
::  traces in verify-inner bubble up as kernel crashes.
::
++  verify-raw
  |=  [=proof override=(unit (list term)) verifier-eny=@ s=* f=*]
  ^-  ?
...
++  verify-door
  ...
  ::
  ::  Phase 1-redo — raw verify that does NOT swallow crashes via mule.
  ::  Lets the trace bubble up for debugging. Not safe to expose to
